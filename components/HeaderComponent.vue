<template>
  <header class="main--header">
    <div class="" style="display: flex; position: relative; align-items: center;">
      <nuxt-link :to="{ name: `index` }" class="main--header__logo">
        <img class="desktop" data-not-lazy :src="require(`~/static/img/logo-mundo_cripto.png`)" />
        <img class="mobile" data-not-lazy :src="require(`~/static/img/mundo-crypto-mobile_logo.png`)" />
        <span class="mcArea">{{mcCurrentSection}}</span>
      </nuxt-link>
      <nav class="main--header__nav ms-4 d-none d-lg-flex">
        ...
        <div class="dropdown-nb">
          <span class="nav-link nav_dropdown">{{ $t('docs') }}</span> <!-- bg-i-all -->
                  ...
                  <div class="row gx-2">
                    <div class="col-7">
                      <div class="tags-links">
                        <div class="item">
                          <a href="https://mundocrypto.com/whitepaper.pdf">{{ $t('docs1') }}</a>
                          <span class="item-separator">/</span>
                        </div>
                      </div>
                      <div class="tags-links">
                        <div class="item">
                          <a href="https://mundocrypto.com/lightpaper.pdf">{{ $t('docs2') }}</a>
                          <span class="item-separator">/</span>
                        </div>
                      </div>
                    </div>
    ...
    <div style="display: flex; position: relative; align-items: center;">
      <nav class="main--header__nav main--header__snav d-none d-lg-flex">
        ...
        <nuxt-link
          :to="'#'"
          class="nav-link class-tv in-active">
          TV <span class="m-hidden">- COMMING SOON</span>
        </nuxt-link>
        <nuxt-link
          :to="'#'"
            class="nav-link">
    ...
    -->
    <div v-if="true" class="position-relative ms-auto">
      <!-- TODO:
    ...
    <span class="main--header__time d-none d-md-block ms-1">{{ time }}</span>

    <!--<div style="display: flex; position: relative; align-items: center;" class="me-4">
      <nav class="main--header__nav main--header__snav d-none d-lg-flex">
        <nuxt-link
          :to="'#'"
          class="nav-link">
        ...
        <nuxt-link
          :to="'#'"
          class="nav-link class-tv"
          @mouseover.native="tvHover = true"
          @mouseleave.native="tvHover = false"
          >
          {{ !tvHover ? 'TV' : 'COMMING SOON' }}
        </nuxt-link>
        <nuxt-link
          :to="'#'"
            class="nav-link">
    ...
    <div class="d-flex justify-content-between align-items-center"  v-if="$auth.$state.loggedIn">
        <div class="walletButton" :class="(userWallet) ? 'active' : 'inactive'">
    ...
    <div v-else class="d-lg-flex align-content-center desktop">
      <div class="d-flex justity-content-center align-items-center" style="height: 30px; width: 30px; justify-content: center; border-radius: 50%; background: #45B2CB">
        <svg width="17" height="15" viewBox="0 0 17 15" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M16.1135 6.40431V5.74774C16.1135 4.54711 15.2896 3.53786 14.1783 3.24867L14.1763 2.16612C14.1763 1.10964 13.3166 0.249878 12.2596 0.249878H12.1583L3.18817 3.03256C2.98387 3.04437 2.79004 3.0916 2.60802 3.16267H2.58462C1.15964 3.16267 0 4.32231 0 5.74774V12.1651C0 13.5903 1.15942 14.7501 2.58462 14.7501H13.5289C14.9539 14.7501 16.1135 13.5905 16.1135 12.1651V11.3116C16.6311 11.1113 17 10.6095 17 10.0218V7.69362C16.9998 7.10633 16.6311 6.6046 16.1135 6.40431ZM15.663 10.0216C15.663 10.0479 15.6423 10.0686 15.616 10.0686H11.9976C11.9713 10.0686 11.9508 10.0479 11.9508 10.0216V7.69362C11.9508 7.66799 11.9718 7.64705 11.9976 7.64705H15.616C15.6419 7.64705 15.663 7.66799 15.663 7.69362V10.0216ZM12.3423 1.59265C12.6232 1.63275 12.8396 1.87471 12.8396 2.16723L12.8413 3.1629H7.28065L12.3423 1.59265ZM13.5289 13.4132H2.58484C1.89575 13.4132 1.33698 12.8542 1.33698 12.1648V5.74774C1.33698 5.0582 1.89575 4.49943 2.58484 4.49943H13.5289C14.2182 4.49943 14.7767 5.0582 14.7767 5.74774V6.31029H11.9976C11.2348 6.31029 10.6141 6.93077 10.6141 7.69362V10.0216C10.6141 10.7846 11.2348 11.4053 11.9976 11.4053H14.7767V12.1651C14.7767 12.8542 14.2182 13.4132 13.5289 13.4132Z" fill="white"/>
        </svg>
      </div>
      <nuxt-link :to="{ name: `auth-login` }" class="main--header__access access d-md-block">
        {{ $t("logIn") }}
      </nuxt-link>

      <nuxt-link :to="{ name: `auth-register` }" class="main--header__access create d-md-block">
        {{ $t("createAccount") }}
      </nuxt-link>
    </div>

    <v-btn icon class="main--header__menu menu--toggle ms-4 d-lg-none" @click.stop="rightDrawer = !rightDrawer">
      <img data-not-lazy src="~/static/img/menu.svg" />
    </v-btn>
    <v-navigation-drawer v-model="rightDrawer" :right="right" fixed :wcategoryth="'100%'">
      <div class="menu--mobile">
        <span class="menu--mobile__close menu--toggle" @click.stop="rightDrawer = false">
          <img data-not-lazy src="~/static/img/close.svg">
        </span>
        <div class="space-1"></div>
        <div class="menu--mobile__nav">
          <span class="menu--mobile__nav__link level-2-link" @click.stop="firstDrawer = !firstDrawer">
            {{ $t("present") }}
          </span>
          <v-navigation-drawer v-model="firstDrawer" :right="right" fixed :wcategoryth="'100%'">
            <div class="menu--mobile__nav">
              <div class="row mb-2">
                <div class="col-6">
                  <span class="menu--mobile__close menu--toggle" @click.stop="firstDrawer = false">
                    <img data-not-lazy src="~/static/img/close.svg">
                  </span>
                  <div class="space-1"></div>
                  <h2>
                    <nuxt-link :to="{ path: `/news/cryptos`, params: { aid: 'cryptos' } }">
                      {{ $t('cryptos') }}
                    </nuxt-link>
                  </h2>
                  <nuxt-link class="menu--mobile__nav__link menu--toggle" :to="{
                    name: `news-aid-slug`, path: `/news/cryptos/bitcoin`,
                    params: { aid: 'cryptos', slug: 'bitcoin', }
                  }">
          ...
          <span class="menu--mobile__nav__link level-2-link" @click.stop="secondDrawer = !secondDrawer">
            {{ $t("market") }}
          </span>
          <v-navigation-drawer v-model="secondDrawer" :right="right" fixed :wcategoryth="'100%'">
            <div class="menu--mobile__nav">
              <!-- TODO: 1-->
              <div class="row mb-2">
                <div class="col-12">
                  <span class="menu--mobile__close menu--toggle" @click.stop="secondDrawer = false">
                    <img data-not-lazy src="~/static/img/close.svg">
                  </span>
                  <nuxt-link class="menu--mobile__nav__link menu--toggle" :to="{
                    name: `aid-slug`,
                  ...
                  <nuxt-link class="menu--mobile__nav__link menu--toggle" :to="{ name: `top-100` }">
          ...
          <div class="others-links">
            <a v-for="{name, url} in mcSections" :href="url" :key="`${name}`">{{name}}</a>
            <a href="#" class="others-links language"><img src="../static/img/world.svg" alt="" class="" />{{locale.code}}</a>
          </div>
...
export default {
  ...
  computed: {
    ...
    shortWallet(){
      let wallet = this.wallet.walletAddress;
      let x = wallet.length;
      let subString = `${wallet.substring(0, 3)}...${wallet.substring(x-3,x)}`
      return subString;
    }
  },
  beforeMount() {
  ...
  created() {
    ...
    // const userModel = await sessionService.user();
    ...
    //  this.time = Intl.DateTimeFormat(code, {
    //    hour: 'numeric',
    //    minute: 'numeric',
    //    second: 'numeric',
    //  }).format()
